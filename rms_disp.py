...
def argparse():
    ...
    parser.add_argument('n_sample', type=int, help='The number of samples to get MSD. Must be an integer larger than 2.')
...
def get_MSD(
    ...
    n_sample (int)
        - The number of samples to get MSD
    ...
    #
    if n_sample < 3:
        raise ValueError('n_sample must be an integer larger than 2')
    ...
    for i in range(len(alist)):
        ...
        MSD[i] = np.mean(np.square(np.linalg.norm(posi[start:end] - np.mean(posi[start:end], axis=0), axis=-1)), axis=0)
...
def plot_MSD(
    ...
    for i in range(len(chem_unique)):
        plt.plot(np.arange(len(avg_MSD[i]))[n_sample//2+1:-n_sample//2-1] *dt, avg_MSD[i][n_sample//2+1:-n_sample//2-1], c=color_list[i], label=chem_unique[i])
    plt.tick_params(axis="both",direction="in", labelsize='x-large')
    plt.title('NS={}, dt={}, GS={}'.format(n_sample, dt, gsmear), fontsize='small')
    plt.xlabel('Time (ns)', fontsize='x-large')
    plt.ylabel('Average MSD ($\AA ^2$)', fontsize='x-large')
    plt.subplots_adjust(left=0.15, bottom=0.25, right=0.95, top=0.75, wspace=0.2, hspace=0.2)
