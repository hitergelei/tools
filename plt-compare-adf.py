...
def argparse():
    ...
    # Optional arguments
    parser.add_argument('-c', '--rectify_cut', type=float, default=0, help='Throw away data of kink larger than cutoff. [Default: 0]')
...
if __name__ == '__main__':
    ...
    plt.rc('font', **font)
    if label_list:
        plt.plot(angle_arr, adf_arr_list[0], label=label_list[0], c='k')
        [plt.plot(angle_arr, adf_arr_list[i], label=label_list[i], linestyle='dashed') for i in range(1,len(adf_arr_list))]
    else:
