...
## Main
curve = np.load('wrapped-gete-conv-md.traj_slice-0-None-1_sym-Ge-Te_nBin-500_rcut-7.1_.npy')
curve = curve[150:300]
x = np.array(curve[:,0])
y_hat = curve[:,1]
...
print(' step |              a              |              mu             |            sigma            |  cost')
print('_____________________________________________________________________________________________________________')
## Train
for i in range(max_step):
    sess.run(optimize)
    if i % print_interval == 0:
        print('{:5d} | {:7.5f}   {:7.5f}           | {:7.5f}   {:7.5f}           | {:7.5f}   {:7.5f}           | {:7.5f}'.format(
...
plt.plot(curve[:,0], np.sum(sess.run(y),axis=0), c='r', linestyle='dashed')
